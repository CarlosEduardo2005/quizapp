...
function alterarAssunto () {
    ...
    const iconeImg = document.querySelector(".assunto_icone_img")
...
async function buscarPerguntas() {
    ...
    await(urlDados).then(resposta => resposta.json()).then(dados => {
...
function alterarSinais(texto) {
    return texto.replace(/</g,"&lt;").repalce(/>g,"&gt;")
}

function guardarResposta() {
    resposta = evento.target.value
    id InputResposta = evento.target.id
...
async function iniciar {
    ...
    const inputsRespostas = document.querySelector(".alternativas input")
